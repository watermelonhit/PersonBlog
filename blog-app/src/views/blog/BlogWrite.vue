  ...
  <div id="write" v-title :data-title="title">
    <el-container>
      ...
      <el-dialog title="摘要 分类 标签"
        ...
        <el-form label-suffix=":" :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <!--       动态标签   后期加入，稳定性暂时未知-->
          <el-form-item label="新建标签" v-model="articleForm.dynamicTags">
            <el-tag
              :key="tag"
              v-for="tag in articleForm.dynamicTags"
              closable
            ...
            <el-input
              class="input-new-tag"
              v-if="articleForm.inputVisible"
              v-model="articleForm.inputValue"
              ref="saveTagInput"
              size="small"
...
import {getAutoSummary,publishArticle, getArticleById} from '@/api/article'
...
export default {
    ...
    return {
      ...
      rules: {
        summary: [
          ...
          {max: 200, message: '不能大于80个字符', trigger: 'blur'}
        ],
        category: [
  ...
  methods: {
    ...
    showInput() {
      this.articleForm.inputVisible = true;
      this.$nextTick(_ => {
    ...
    handleInputConfirm() {
      let inputValue = this.articleForm.inputValue;
      if (inputValue) {
        this.articleForm.dynamicTags.push(inputValue);
      }
      this.articleForm.inputVisible = false;
      this.articleForm.inputValue = '';
    },
    getArticleById(id) {
      let that = this
    ...
    publishShow() {
      ...
      // 若勾选自动生成summary,则传送文章title和content给后端
      if(this.autoSummary){
         this.toGetAutoSummary()
      }
    },
    toGetAutoSummary(){
      let article = {
      ...
      getAutoSummary(article, this.$store.state.token).then((data) => {
        if (data.success) {
          loading.close();
          this.articleForm.summary=data.data
          this.$message({message: '成功自动生成文章摘要！', type: 'success', showClose: true})
        } else {
          this.$message({message: '标题数不能超过200，文章内容数不能超过3000！', type: 'error', showClose: true});
        }
      }).catch((error) => {
        loading.close();
        if (error !== 'error') {
